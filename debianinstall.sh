...
while true; do
    ...
    if [ -z "$password" ]; then
	    echo ""
        echo -e "\033[1;31mSenha não pode estar vazia.\033[0m" # Mensagem de erro em vermelho
    elif [ "${#password}" -lt 8 ]; then
	    echo ""
        echo -e "\033[1;31mSenha deve ter pelo menos 8 caracteres.\033[0m" # Mensagem de erro em vermelho
    else
        break
    fi
done
...
set_mirror_proxy() {
    ...
    case $mirror_protocol in
        ...
        ftp)
            ...
            ;;
        *)
            err "Protocol não suportado: $mirror_protocol"
    esac
}
...
has_cloud_kernel() {
    ...
    warn "Nenhum kernel de nuvem está disponível para $architecture/$suite$tmp"
...
has_backports() {
    ...
    warn "Nenhum kernel de backports está disponível para $suite"
...
while true; do
    clear
    echo -e "\033[1;31m═══════════════════════════════════════════════\033[0m"
    tput setaf 8 ; tput setab 4 ; tput bold ; printf '%36s%s%-12s\n' "ESCOLHA A VERSÃO DO DEBIAN" ; tput sgr0
    echo -e "\033[1;31m═══════════════════════════════════════════════\033[0m"
    echo ""
    echo -e "\033[1;32mRecomenda-se a versão 11 do Debian.\033[0m"
	echo ""
    echo -ne "Escolha (10, 11, 12, 13) [Padrão: 11]: "
    read -r user_input

    # Usar a versão padrão se o usuário não digitar nada
    debian_version=${user_input:-$debian_version}

    if [[ "$debian_version" =~ ^(10|11|12|13|buster|bullseye|bookworm|trixie|oldoldstable|oldstable|stable|testing|sid|unstable)$ ]]; then
        break
    else
...
if [ "$account_setup" = true ]; then
    ...
    prompt_password "Escolha uma senha para o usuário instalador do console de rede SSH: "
fi
